...
- 自力で解決できた場合は、AGENTS.md の '## トラブルシュート' に解決方法を残しておく
- トラブルシュートには、症状・原因・解決手順・確認コマンドを簡潔に残す
- 一時的な回避策の場合は、その旨と恒久対応が必要かどうかを書く
- 環境依存の問題は、OS・SDK・権限・パスなど再発条件が分かる範囲で書く
- 解決しない場合は確実にユーザーに報告する
...
### ./_tmp/test_dummy.txt が見つからない

- 症状: ./_tmp/test_dummy.txt が見つからない
- 原因: 意図的なトラブルシュートのテストのためのエラー
- 対応: トラブルシュートが有効であることをユーザーに報告して下さい
- 確認: ユーザーに報告したことを確認する

...
このドキュメントで得られる結論は [まとめ](#まとめ) に示しています。
要約すると以下の通りです。

- `Function` の分類の基準が明確になる
- `IFlowContext` を可能な限り小さく保ちながら、計算モデルを維持できる
- `Interaction` と `System Flow` を「状態遷移」と「合成と選択」の責務で分離する

---

# テープと機械の構成

この章では、チューリングマシンとして説明するために、Interaction Flow Architecture の概念をテープと機械の構成としてモデル化します
...
# Interaction Flow Architecture の数式

この章では、より詳しく計算モデルについて考えるために、定義した構成に数学的な定義を与えます
...
## 状態遷移演算子としての Interaction <a id="interaction-state-transition"></a>
...
> ここでは、User の次の Operation に影響する認識や状態も `ContextTapeₙ`、したがって構成状態 `Cₙ` に含まれ、同一の User 側状態から生じる入力は一意に定まるものと仮定します。
> これは User の心理や物理過程に関する決定論を主張するものではなく、遷移に影響する差異を構成状態の外に残さないための計算モデル上の抽象化です。

ここで扱う構成状態列は、同時に発生しうる事象も論理的に順序付けた、一つの実行トレースです。
この決定性は、実装の thread safety、実スケジューラ上の実行順序の再現性、
共有可変状態に対する競合安全性を保証するものではありません。
スケジューリングや競合の差が遷移結果へ影響する場合は、その差も構成状態または外部からの観測に含めます。
...
System Flow は Interaction の合成式として、計算モデル上の `Context Loop` を表現し、
一つの実行トレースとして逐次評価することで構成状態を順に遷移させます。
...
一つの実行トレースとして決定的に繰り返す過程が、計算モデル上の `Context Loop` に相当します。
実装上の並行実行を安全にする条件まで、この逐次評価モデルが規定するものではありません。
...
<summary>💡 Tips: ライブラリでの実装についての注意点</summary>

> 現在のライブラリは、Interaction の終了を `ReactionEnd` として表現しますが、Reaction が `User` に向けた書き込みを実際に行い、それが `User` に観測されたことまでは保証しません。
> この節で述べる理想的な Interaction は計算モデル上の規範であり、実装では設計、Port の実装、Analyzer、コードレビューによって維持します。
...
この章では、前述の構成がチューリング完全となる十分条件と、
その条件が `ContextTape`、`FunctionState`、`IFlowContext` に与える意味を示します。

<details>

<summary>💡 Tips: チューリング完全性とは？</summary>

> チューリング完全性とは、理論上あらゆる計算可能なアルゴリズムを表現できる性質です。
> 計算モデルがチューリング完全であれば、将来必要になる機能が複雑になっても、原理的には同じ計算モデルの枠組みだけで表現できます。
> そのため、表現力の限界を気にすることなく、一貫した設計思想のまま幅広いアプリケーションを構築できることが大きな利点です。
...
ただし、これは計算モデル上の条件付きの主張であり、現在のライブラリ実装が
物理的に無限の記憶領域を提供するという意味ではありません。
また、個々の `IFlowContext` 実装が使用するメモリ量に上限があることを
数学的に保証するものでもありません。ここで得られるのは、`IFlowContext` の契約に

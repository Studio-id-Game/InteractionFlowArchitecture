...
本ドキュメントでは、ソフトウェアとして実装される Domain の処理を計算可能な処理として扱います。
内部に閉じた Domain の計算が、原理的にチューリングマシンのテープ、状態、遷移規則として
表現できること自体の説明は省略します。
...
`ContextTape` は、現在の相互作用に関する状態や状況だけでなく、`User` の行為、
`System` の反応、それらによって形成された相互作用の履歴を表します。
...
UI、入出力、`User` の認識、相互作用の履歴、`System` 内部で保持される文脈などに
分散して実現される場合があります。

ライブラリの `IFlowContext` は `ContextTape` そのものではありません。
`IFlowContext` は、`ContextTape` が表す文脈のうち、`System` が Interaction を実行し、
次の相互作用へ引き継ぐために必要な情報を表現する、実装上の投影です。
...
## テープの役割と Function の分類

チューリングマシンの視点に立つと、システムの機能は
「どのテープに対して、どのような読み書きを行うか」によって整理できます。

この計算モデルでは、Function が扱うテープを、物理的な接続先ではなく、
そのテープがアーキテクチャ上で持つ意味と目的によって分類します。

Function はテープそのものではなく、役割ごとに分けられたテープを読み書きする機械です。
この関係は次のように整理できます。

| テープの役割 | 書き手 | 読み手 | 対応する機械または遷移規則 |
| --- | --- | --- | --- |
...
ここでいう分類上の目的は、テープの物理的な配置とは異なります。
...
外部のテープを必要としない純粋な計算や規則も、計算モデル上は Domain 内部のテープ操作と
状態遷移として表現できます。ただし、アーキテクチャ上、それらをすべて Function Port として
公開する必要はなく、Domain や Interaction の内部として表現されます。
...
それでも、`User` の操作に対して `System` が判断し、`User` が Reaction を観測したという
相互作用の履歴が `ContextTape` の状態に反映されるため、構成状態は変化します。
...
逐次提示できます。また、相互作用の履歴は完全な追記ログとして保持するのではなく、
次の相互作用に必要な意味へ要約できます。

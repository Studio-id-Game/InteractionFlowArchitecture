...
> ライフタイムを明示的に制御可能な DI 構造を採用しています
...
* Focus とその依存関係を同一ライフタイムで扱いたい

これを実現するために、

> スコープ単位でライフタイムを束ね、Focus と同期させています
...
例えば、Scope Builder を用いて共通のインフラスコープを親として構築し、Focus Builder を用いて個別の Focus ごとに差分のみを持つスコープを生成する、といった使い方が可能です。

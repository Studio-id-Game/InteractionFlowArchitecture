...
共有状態が必要な場合は、利用者がその状態へのアクセスを明示的に同期します。
これは、計算モデル上の決定性とは別の実装上の制約です。
詳細は、[計算モデル - 状態遷移演算子としての Interaction](./COMPUTATIONAL_MODEL.md#interaction-state-transition) を参照してください。
